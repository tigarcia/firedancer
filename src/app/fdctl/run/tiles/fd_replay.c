...
after_frag( void *             _ctx,
  ...
  if ( in_idx == GOSSIP_IN_IDX ) {
    ...
    /* for now, just modify the timestamp and echo the vote txn back with gossip */
    ...
    fd_voter_t voter = {
      ...
      .node_pubkey               = ctx->node_identity,
      .authorized_voter_pubkey   = ctx->node_identity,
      .voter_sign_arg            = ctx,
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->node_identity  = (fd_pubkey_t *) fd_type_pun_const( identity_pubkey );
