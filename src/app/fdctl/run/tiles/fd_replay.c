...
scratch_footprint( fd_topo_tile_t const * tile FD_PARAM_UNUSED ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_ghost_align(), fd_ghost_footprint( 1 << FD_BFT_LG_SLOT_MAX, 1 << FD_BFT_LG_NODE_PUBKEY_MAX ) );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  void * ghost_mem           = FD_SCRATCH_ALLOC_APPEND( l, fd_ghost_align(), fd_ghost_footprint( 1 << FD_BFT_LG_SLOT_MAX, 1 << FD_BFT_LG_NODE_PUBKEY_MAX ) );
  ulong  scratch_top         = FD_SCRATCH_ALLOC_FINI  ( l, scratch_align() );
  if( FD_UNLIKELY( scratch_top != ( (ulong)scratch + scratch_footprint( tile ) ) ) ) {
    FD_LOG_ERR(( "scratch alloc does not match footprint %lu %lu %lu", scratch_top - (ulong)scratch - scratch_footprint( tile ), scratch_top, (ulong)scratch + scratch_footprint( tile ) ));
  }
