...
vote_txn_signer( void *        keypair,
                 ...
                 ulong         len ) {
    fd_sha512_t sha;
    uchar * validator_keypair = (uchar*) fd_type_pun( keypair );
    fd_ed25519_sign( /* sig */ signature,
                     /* msg */ buffer,
                     /* sz  */ len,
                     /* public_key  */ validator_keypair + 32UL,
                     /* private_key */ validator_keypair,
                     &sha );
}
...
main( int argc, char ** argv ) {
  ...
  fd_pubkey_t * validator_pubkey = (fd_pubkey_t *) fd_type_pun_const( validator_keypair + 32 );
  ...
  fd_voter_t voter = {
    ...
    .authorized_voter_pubkey   = validator_pubkey,
    .voter_sign_arg            = validator_keypair,
    .node_sign_fun             = vote_txn_signer,
    .authorized_voter_sign_fun = vote_txn_signer
  };
