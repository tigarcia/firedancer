...
fd_vote_txn_generate( fd_voter_t *                     voter,
  ...
  uchar authorized_voter_is_node_identity = ( memcmp( voter->node_pubkey->key,
                                                      voter->authorized_voter_pubkey->key,
                                                      32 ) == 0 );
  if ( FD_LIKELY( authorized_voter_is_node_identity ) ) {
    ...
    vote_txn_accounts.signers_w             = voter->node_pubkey;
    ...
    vote_txn_accounts.signers_w             = voter->node_pubkey;
    vote_txn_accounts.signers_r             = voter->authorized_voter_pubkey;
    vote_txn_accounts.non_signers_w         = voter->vote_acct_addr;
    vote_txn_accounts.non_signers_r         = vote_program_addr;
    FD_TEST( fd_txn_base_generate( out_txn_meta_buf, out_txn_buf, num_signatures, &vote_txn_accounts, recent_blockhash ) );
  }
  ...
  if ( FD_LIKELY( authorized_voter_is_node_identity ) ) {
    ...
    voter->node_sign_fun( voter->voter_sign_arg,
    ...
    voter->node_sign_fun( voter->voter_sign_arg,
                          ...
                          /* buffer */ buf_start,
                          /* len */    buf_size);
    voter->authorized_voter_sign_fun( voter->voter_sign_arg,
